...
namespace OurWord.DataModel
{
	...
	public class DBasicText : DRun
	{
		...
		private void AppendPhrases(DPhrases osPhrases, DPhrases phrasesToAppend)
		{
			DPhrase LastPhrase = ( osPhrases.Count > 0 ) ? 
				osPhrases[ osPhrases.Count - 1] : null;


			// Add a space to our last phrase if needed
			if ( null != LastPhrase && ! LastPhrase.EndsWithSpace &&
				phrasesToAppend.Count > 0 && ! phrasesToAppend[0].BeginsWithSpace)
			{
			...
			while (phrasesToAppend.Count > 0)
			{
				...
				if (null != LastPhrase && 
					phrase.CharacterStyleAbbrev == LastPhrase.CharacterStyleAbbrev)
				{
		...
		public virtual void Append(DBasicText text)
		{
			// Append the vernacular and BT phrases
			AppendPhrases( Phrases,   text.Phrases);
			AppendPhrases( PhrasesBT, text.PhrasesBT);
		}
		#endregion
	...
	public class DText : DBasicText
	{
		...
		public override void Append(DBasicText basicText)
		{
			...
			while (text.Notes.Count > 0)
			{
				...
				noteHome.NoteText.Append( note.NoteText );
				text.Notes.Remove(note);
			}
		}
		#endregion
	...
	public class Test_DRun : Test
	{
		...
		public Test_DRun()
			: base("DRun")
		{
			AddTest( new IndividualTest( Test_GetPhrase ),       "Get Phrase" );
			AddTest( new IndividualTest( Test_FieldToRuns ),     "Field-To-Runs" );
			AddTest( new IndividualTest( Test_EliminateSpaces ), "Eliminate Spurious Spaces" );
			AddTest( new IndividualTest( Test_TextToWordList ),  "Text To Word List" );
		}
		#endregion

		// Test GetPhrase---------------------------------------------------------------------
		#region Method: void AreSame(Phrase p, sStyleAbbrev, sText)
		void AreSame(DSection.IO.Phrase p, string sStyleAbbrev, string sText)
		{
			AreSame(p.StyleAbbrev, sStyleAbbrev);
			AreSame(p.Text,        sText);
		}
		#endregion
...
//			EnableTracing = true;
			...
			int iPos = 0;
			AreSame( DSection.IO.Phrase.GetPhrase(s, ref iPos), 
		...
		void CheckText( DRun[] runs, int iRun, int iPhrase, string sTextExpected)
		{
			...
			AreSame(sTextExpected, phrase.Text);
		}
		#endregion
		...
		#endregion
		#region Test: Test_FieldToRuns
		public void Test_FieldToRuns()
		{
...
//			CheckBT(  runs, 0, 0, "");
		...
		#region Test: Test_EliminateSpaces
		public void Test_EliminateSpaces()
		{
		...
		// WordList --------------------------------------------------------------------------
		#region Test: Test_TextToWordList
		public void Test_TextToWordList()
		{
			...
			AreSame( "A",         aWords[ 0] as string);  AreSame(  0, (int)aPositions[ 0]);
			AreSame( "hard",      aWords[ 1] as string);  AreSame(  2, (int)aPositions[ 1]);
			AreSame( "worker",    aWords[ 2] as string);  AreSame(  7, (int)aPositions[ 2]);
			AreSame( "has",       aWords[ 3] as string);  AreSame( 14, (int)aPositions[ 3]);
			AreSame( "plenty",    aWords[ 4] as string);  AreSame( 18, (int)aPositions[ 4]);
			AreSame( "of",        aWords[ 5] as string);  AreSame( 25, (int)aPositions[ 5]);
			AreSame( "food,",     aWords[ 6] as string);  AreSame( 28, (int)aPositions[ 6]);
			AreSame( "but",       aWords[ 7] as string);  AreSame( 34, (int)aPositions[ 7]);
			AreSame( "a",         aWords[ 8] as string);  AreSame( 38, (int)aPositions[ 8]);
			AreSame( "person",    aWords[ 9] as string);  AreSame( 40, (int)aPositions[ 9]);
			AreSame( "who",       aWords[10] as string);  AreSame( 47, (int)aPositions[10]);
			AreSame( "chases",    aWords[11] as string);  AreSame( 51, (int)aPositions[11]);
			AreSame( "fantasies", aWords[12] as string);  AreSame( 58, (int)aPositions[12]);
			AreSame( "ends",      aWords[13] as string);  AreSame( 68, (int)aPositions[13]);
			AreSame( "up",        aWords[14] as string);  AreSame( 73, (int)aPositions[14]);
			AreSame( "in",        aWords[15] as string);  AreSame( 76, (int)aPositions[15]);
			AreSame( "poverty.",  aWords[16] as string);  AreSame( 79, (int)aPositions[16]);

			// Pathalogical example
			text = DText.CreateSimple("This . is a pathalogical ,!: case. ");
			text.GetWordOffsetPairs(ref aWords, ref aPositions);
//			foreach(string s in aWords)
//				Console.WriteLinw("WORD = \"" + s + "\"");
//			foreach(int n in aPositions)
//				Console.WriteLine(n.ToString());
			AreSame( "This",         aWords[ 0] as string);  AreSame(  0, (int)aPositions[ 0]);
			AreSame( "is",           aWords[ 1] as string);  AreSame(  7, (int)aPositions[ 1]);
			AreSame( "a",            aWords[ 2] as string);  AreSame( 10, (int)aPositions[ 2]);
			AreSame( "pathalogical", aWords[ 3] as string);  AreSame( 12, (int)aPositions[ 3]);
			AreSame( "case.",        aWords[ 4] as string);  AreSame( 29, (int)aPositions[ 4]);
		}
		#endregion
	}
	#endregion
