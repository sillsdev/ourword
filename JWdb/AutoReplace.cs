...
namespace JWdb
{
	...
	#region TEST
	public class Test_AutoReplace : Test
	{
		#region Constructor()
		public Test_AutoReplace()
			: base("AutoReplace")
		{
			AddTest( new IndividualTest( AddThenSearch ), "AddThenSearch" );
		}
		#endregion
		...
		public void AddThenSearch()
		{
			...
			AreSame( Source.Length, Result.Length);
			int cSourceLen = 0;
			...
			for(int i=0; i<Source.Length; i++)
			{
				AreSame( Result[i], tr.Search( Source[i], ref cSourceLen ) );
				AreSame( Source[i].Length, cSourceLen );
			}

			// Do we NOT find stuff we shouldn't?
			IsNull( tr.Search( "repair", ref cSourceLen) );
			IsNull( tr.Search( "biki", ref cSourceLen) );
			IsNull( tr.Search( "men", ref cSourceLen) );
			IsNull( tr.Search( "Bikin", ref cSourceLen) );

			// We should find something for contained strings
			AreSame( "help",  tr.Search( "saya mau menolong", ref cSourceLen ) );
			AreSame( "drink", tr.Search( "jangan minum", ref cSourceLen ) );

			// Should differentiate between capitalization
			IsNotNull( tr.Search( "menulis", ref cSourceLen ) );
			IsNull( tr.Search( "Menulis", ref cSourceLen ) );
