...
namespace JWdb.DataModel
{
	...
	public class DSection : JObject
	{
        ...
        public class MergeMethod
        {
            ...
            #endregion
            #region Method: void ConsoleWriteDiffItem(Diff.Item)
            void ConsoleWriteDiffItem(Diff.Item item)
            {
            ...
            #endregion
            #region Method: void ConsoleWriteDiff(sA, sB, Diff.Item[])
            void ConsoleWriteDiff(string sA, string sB, Diff.Item[] v)
            {
                ...
                string GetNoteText()
                {
                    ...
                    if (Item.deletedA == 0 && Item.insertedB > 0)
                    {
                        ...
                        return LocDB.GetValue(
                            new string[] { "Strings" },
                            "mergeInsertion",
                            "The other versioon inserted \"{0}\" at \"{1}\".",
                            null,
            ...
            #region Method: void ReconcileTextualChanges()
            void ReconcileTextualChanges()
            {
                ...
                ConsoleWriteDiff(sMine, sTheirs, vDiffItems);
                ...
                foreach (DiffItemEx itemx in vItemsEx)
                {
                    ...
                    // TODO NEXT: Get thte closes DText to append this note to; would be
                    // MyRun if it is a DText, otherwise must find a DText
//                    textMine.TranslatorNotes.Append(note);
                }
            ...
            public void Run()
            {
                ...
                // Use Diff to reconcile the textual changes
                ReconcileTextualChanges();
            }
            #endregion
        }
        #endregion
	...
	public class Test_DSection : Test
	{
        ...
        static public void CompareSections(Test t, DSection s1, DSection s2)
		{
			...
			for(int i=0; i < s1.Paragraphs.Count; i++)
			{
				...
				if (DB.TeamSettings.SFMapping.IsVernacularParagraph(pg1.StyleAbbrev))
				{
			...
			// are not disappearing.
		{
            ...
            Book1.Write(new NullProgress());

			// Read it into Book2
            Book2.Load(new NullProgress());
...
//			EnableTracing = true;
			...
			for(int i=0; i<SNew.Paragraphs.Count; i++)
			{
				...
				AreSame(sExp, sAct);
				if (sAct == DB.TeamSettings.SFMapping.StyleCrossRef)
					iCrossRefPara = i;
			}
