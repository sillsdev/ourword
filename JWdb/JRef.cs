...
namespace JWdb
{
	...
	#region TEST
	public class Test_JRef : Test
	{
		#region Constructor()
		public Test_JRef()
			: base("JRef")
		{
			AddTest( new IndividualTest( PathToReferencedObject ), "PathToReferencedObject" );
//			AddTest( new IndividualTest( RefObjReadWrite ),        "RefObjReadWrite" );
		}
		#endregion
		#region Attribute: ReadWritePathName - pathname for test file
		private string ReadWritePathName
		{
			get 
			{
				string sPath = Application.ExecutablePath;
				sPath = sPath.Substring(0, sPath.LastIndexOf("\\") ) + "\\RefTest.x";
				return sPath;
			}
		}
		#endregion

		#region PathToReferencedObject
		public void PathToReferencedObject()
		{
			TObjA objA = new TObjA("A");
			TObjC objC = objA.ObjC;
			TObjD objD = objA.ObjD;
			TObjE objE = objA.ObjE;

			string sPathFromAtoD = objA.m_ref1.PathToReferencedObject;
			Trace("PathFromAtoD = " + objA.m_ref1.PathToReferencedObject);
			AreSame("0-A1-0-B-0-COwn", sPathFromAtoD);

			string sPathFromDtoE = objD.m_ref1.PathToReferencedObject;
			Trace("PathFromDtoE = " + objD.m_ref1.PathToReferencedObject);
			AreSame("1-COwn", sPathFromDtoE );

			string sPathFromDtoA = objD.m_ref2.PathToReferencedObject;
			Trace("PathFromDtoA = " + objD.m_ref2.PathToReferencedObject);
			AreSame("3", sPathFromDtoA );
		}
		#endregion

        /***QQQQ
		#region RefObjReadWrite
		public void RefObjReadWrite()
			...
			// If this works, there is a lot that is working correctly to get to this point.
		{
			...
			SS_out.Write(ReadWritePathName);

			// Read it into a blank style sheet
			JStyleSheet SS_in = new JStyleSheet();
			string sReadWritePathName = ReadWritePathName;
